      ...
      <div className="flex flex-col md:flex-row items-start justify-start md:justify-between md:items-center mb-4">
        ...
        {projeto.informacoes && (
          <button
            ...
            className="text-sm bg-primary text-white px-2 py-1 mt-2 rounded hover:bg-primary/90 transition-colors"
          >
            {showInfo
              ? 'Ocultar informações'
              : 'Confira informações sobre o projeto'}
          </button>
        )}
      </div>
      {showInfo && projeto.informacoes && (
        <div className="max-w-3xl overflow-hidden mb-4 p-4 bg-gray-50 rounded-lg">
          <div
            className="text-gray-700 prose max-w-3xl"
            dangerouslySetInnerHTML={{ __html: projeto.informacoes }}
          />
        </div>
      )}
